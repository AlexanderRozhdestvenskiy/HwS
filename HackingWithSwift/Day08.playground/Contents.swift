...
enum PasswordErrorTwo: Error {
    case low
    case higth
}

func passwordIn(password: String) throws -> String {
    guard password.count > 4 else { throw PasswordErrorTwo.low }
    guard password.count < 10 else { throw PasswordErrorTwo.higth }
    
    return password
}

do {
    let final2 = try passwordIn(password: " ")
    print(final2)
} catch PasswordErrorTwo.low {
    print("Low")
} catch PasswordErrorTwo.higth {
    print("Higth")
} catch {
    print("?")
}
